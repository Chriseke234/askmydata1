...
Would you like me to open a deep investigation or build a custom executive dashboard?`,
  ...
  return (
    <div className="min-h-screen bg-[#0B0C0E] text-white">
      {/* Demo Header */}
      <header className="sticky top-0 z-40 bg-[#0B0C0E]/90 backdrop-blur border-b border-[#1A1D24] px-4 sm:px-8 py-3 flex items-center justify-between">
        <Link href="/" className="flex items-center space-x-3">
          ...
          <span className="font-bold text-lg text-white">ASKMYDATA</span>
          <span className="px-2 py-0.5 rounded bg-[#D4AF37]/20 border border-[#D4AF37] text-[10px] text-[#D4AF37] font-bold uppercase">
            Live Demo Mode
          </span>
        </Link>

        <div className="flex items-center space-x-3">
          <span className="hidden sm:inline text-xs text-[#9CA3AF]">Active Dataset: <strong className="text-white">Northstar Commerce</strong></span>
          <Link href="/login" className="px-4 py-2 bg-[#D4AF37] hover:bg-[#E5B800] text-black font-bold rounded-lg text-xs transition-colors gold-glow">
            Sign Up Free
          </Link>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-8 py-8 space-y-8">
        ...
        <form onSubmit={(e) => { e.preventDefault(); if (question.trim()) handleAsk(question); }} className="sticky bottom-4 z-30 flex items-center p-2 bg-[#121417] border border-[#D4AF37]/50 rounded-xl shadow-2xl gold-border-glow">
          <Sparkles className="w-5 h-5 text-[#D4AF37] ml-2 shrink-0" />
          <input
            type="text"
            value={question}
